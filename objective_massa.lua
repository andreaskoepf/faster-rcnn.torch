  ...
  local function lossAndGradient(w)
    ...
    for i,x in ipairs(batch) do
      ...
      for i,x in ipairs(n) do
        ...
        -- extract roi pooling input
        if mode ~= 'onlyPnet' then
          local outpnetCLS = v[{{1, 2}}]
          local softmax = nn.SoftMax():cuda()
          local c_prop = softmax:forward(outpnetCLS)
          if c_prop[1] > c_prop[2] then -- take negatives if and only if pnet has classified them as positive
            --local reg_out = v[{{3, 6}}] -- Anchor  
            --local reg_proposal = Anchors.anchorToInput(anchor, reg_out) --reg_out
            --local pi, idx = extract_roi_pooling_input(reg_proposal, localizer, outputs[#outputs])
      ...
      creg_count = creg_count + #p - counter
      ccls_count = ccls_count + #p - counter + negative_counter
    ...
    if ccls_count ~= 0 then
       gradient:div(ccls_count) -- will divide all elements of gradient with ccls_count in-place
    end
    --if creg_count ~= 0 then
    --   gradient:div(creg_count) -- will divide all elements of gradient with creg_count in-place
    --end
