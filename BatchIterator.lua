...
function BatchIterator:processImage(img, rois)
  ...
  local tw, th
  if not cfg.uniform_image_scaling then
    tw, th = find_target_size(img_size[3], img_size[2], cfg.target_smaller_side, cfg.max_pixel_size)
  else
    tw, th = find_scaled_size(img_size[3], img_size[2], cfg.target_smaller_side)
  end
